...
bool CodeGenFunction::EmitScalarRangeCheck(llvm::Value *Value, QualType Ty,
  ...
  if (Value->getType()->isByteTy())
    Value = Builder.CreateByteCastToInt(Value, "", true);
    // return false;
  ...
  if (!Dst.isSimple()) {
    if (Dst.isVectorElt()) {
      ...
      if (VecIdx->getType()->isByteTy())
        VecIdx = Builder.CreateByteCastToInt(VecIdx, "", /*IsExact=*/true);
      llvm::errs() << "Dst.getVectorIdx(): " << *Dst.getVectorIdx() << "\n";
  ...
  auto EmitIdxAfterBase = [&, IdxPre](bool Promote) -> llvm::Value * {
    ...
    if (Promote && Idx->getType() != IntPtrTy) {
      if (Idx->getType()->isByteOrByteVectorTy())
        Idx = Builder.CreateByteCastToInt(Idx, "idxprom", /*IsExact=*/true);
