...
void CodeGenFunction::EmitBoundsCheckImpl(const Expr *ArrayExpr,
  ...
  if (BoundsVal->getType()->isByteTy())
    BoundsVal = Builder.CreateByteCastToInt(BoundsVal);
  ...
  if (!Dst.isSimple()) {
    if (Dst.isVectorElt()) {
      ...
      if (VecIdx->getType()->isByteTy())
        VecIdx = Builder.CreateByteCastToInt(VecIdx);
...
void CodeGenFunction::EmitStoreThroughBitfieldLValue(RValue Src, LValue Dst,
  ...
  if (SrcVal->getType()->isByteTy())
    SrcVal = Builder.CreateByteCastToInt(SrcVal);
...
llvm::Value *CodeGenFunction::EmitCheckValue(llvm::Value *V) {
  ...
  if (V->getType()->isByteTy(8) &&
    ...
    V = Builder.CreateByteCast(V, IntTy);
    return Builder.CreateZExt(V, TargetTy);
  }
  ...
  auto EmitIdxAfterBase = [&, IdxPre](bool Promote) -> llvm::Value * {
    ...
    if (Promote && Idx->getType() != IntPtrTy) {
      if (Idx->getType()->isByteOrByteVectorTy())
        Idx = Builder.CreateByteCastToInt(Idx, "idxprom");
  ...
  if (IsLowerBound || E->getColonLocFirst().isInvalid()) {
    ...
    if (auto *LowerBound = E->getLowerBound()) {
      ...
      if (LowerBoundVal->getType()->isByteTy())
        LowerBoundVal = Builder.CreateByteCastToInt(LowerBoundVal);
