  ...
  for (Function::const_arg_iterator ai = F->arg_begin(), ae = F->arg_end();
    ...
    if (shift > 0 && shift == j)
      ...
      Ops[j] = Ops[j]->getType()->isByteOrByteVectorTy() &&
        ...
        ? Builder.CreateExactByteCast(Ops[j], ai->getType(), name)
...
Value *CodeGenFunction::EmitNeonRShiftImm(Value *Vec, Value *Shift,
  ...
  if (Vec->getType()->isByteOrByteVectorTy())
    Vec = Builder.CreateExactByteCastToInt(Vec);
...
static Value *EmitCommonNeonSISDBuiltinExpr(
  ...
  for (Function::const_arg_iterator ai = F->arg_begin(), ae = F->arg_end();
    ...
    if (Ops[j]->getType()->isByteTy())
      Ops[j] = CGF.Builder.CreateExactByteCast(Ops[j], ArgElTy);
...
Value *CodeGenFunction::EmitCommonNeonBuiltinExpr(
  ...
  case NEON::BI__builtin_neon_splatq_laneq_v: {
    ...
    if (Ops[0]->getType()->isByteOrByteVectorTy())
      Ops[0] = Builder.CreateExactByteCastToInt(Ops[0]);
  ...
  case NEON::BI__builtin_neon_vaddq_v: {
    ...
    Ops[0] = Ops[0]->getType()->isByteOrByteVectorTy()
      ? Builder.CreateExactByteCast(Ops[0], VTy)
      : Builder.CreateBitCast(Ops[0], VTy);
    Ops[1] = Ops[1]->getType()->isByteOrByteVectorTy()
      ? Builder.CreateExactByteCast(Ops[1], VTy)
  ...
  case NEON::BI__builtin_neon_vaddhn_v: {
    ...
    if (Ops[1]->getType()->isByteOrByteVectorTy())
      Ops[1] = Builder.CreateExactByteCastToInt(Ops[1]);
  ...
  case NEON::BI__builtin_neon_vfmaq_v: {
    ...
    if (Ops[2]->getType()->isByteOrByteVectorTy())
      Ops[2] = Builder.CreateExactByteCastToInt(Ops[2]);
  ...
  case NEON::BI__builtin_neon_vld4q_lane_v: {
    ...
    for (unsigned I = 2; I < Ops.size() - 1; ++I) {
      if (Ops[I]->getType()->isByteOrByteVectorTy())
        Ops[I] = Builder.CreateExactByteCastToInt(Ops[I]);
...
CodeGenFunction::EmitAArch64CompareBuiltinExpr(Value *Op, llvm::Type *Ty,
  ...
  if (isa<FixedVectorType>(Ty)) {
    ...
    Op = Op->getType()->isByteOrByteVectorTy()
      ? Builder.CreateExactByteCast(Op, Ty)
...
Value *CodeGenFunction::EmitARMBuiltinExpr(unsigned BuiltinID,
  ...
  if (BuiltinID == clang::ARM::BI__builtin_arm_strex ||
    ...
    if (StoreVal->getType()->isPointerTy())
      ...
      StoreVal = StoreVal->getType()->isByteTy()
                     ? Builder.CreateExactByteCast(StoreVal, IntTy)
  ...
  if (CRCIntrinsicID != Intrinsic::not_intrinsic) {
    ...
    if (BuiltinID == clang::ARM::BI__builtin_arm_crc32d ||
      ...
      if (Arg1->getType()->isByteTy())
        Arg1 = Builder.CreateExactByteCastToInt(Arg1);
  ...
  case NEON::BI__builtin_neon_vld1q_lane_v:
    ...
    if (VTy->getElementType()->isIntegerTy(64)) {
      ...
      Ops[1] = Builder.CreateExactByteCast(Ops[1], Ty);
  ...
  case NEON::BI__builtin_neon_vrsraq_n_v:
    Ops[0] = Builder.CreateExactByteCast(Ops[0], Ty);
    Ops[1] = Builder.CreateExactByteCast(Ops[1], Ty);
...
Value *CodeGenFunction::EmitSVEScatterStore(const SVETypeFlags &TypeFlags,
  ...
  // folded into a nop.
  if (Ops[0]->getType()->isByteOrByteVectorTy())
    Ops[0] = Builder.CreateExactByteCast(Ops[0], OverloadedTy);
...
Value *CodeGenFunction::EmitSVEMovl(const SVETypeFlags &TypeFlags,
  ...
  if (Ops[0]->getType()->isByteOrByteVectorTy() &&
    ...
    return Builder.CreateCall(F,
                              {Builder.CreateExactByteCastToInt(Ops[0]),
                               Builder.getInt32(0)});
...
Value *CodeGenFunction::EmitSVEMaskedLoad(const CallExpr *E,
  ...
  if (Load->getType()->isByteOrByteVectorTy() &&
    ...
    LoadVal = Builder.CreateExactByteCastToInt(Load);
...
Value *CodeGenFunction::EmitSMEReadWrite(const SVETypeFlags &TypeFlags,
  ...
  for (unsigned i = 0; i < FTy->getNumParams(); ++i)
    if (Ops[i]->getType()->isByteOrByteVectorTy() &&
      ...
      Ops[i] = Builder.CreateExactByteCast(Ops[i], FTy->getParamType(i));
...
Value *CodeGenFunction::EmitSVEReinterpret(Value *Val, llvm::Type *Ty) {
  ...
  if (auto *StructTy = dyn_cast<StructType>(Ty)) {
    ...
    for (unsigned I = 0; I < StructTy->getNumElements(); ++I) {
      ...
      Value *Out = In->getType()->isByteOrByteVectorTy() &&
        ...
        ? Builder.CreateExactByteCast(In, StructTy->getTypeAtIndex(I))
  ...
  return Val->getType()->isByteOrByteVectorTy() && !Ty->isByteOrByteVectorTy()
    ? Val = Builder.CreateExactByteCast(Val, Ty)
  ...
  if (TypeFlags.isLoad())
    ...
    if (TypeFlags.hasSplatOperand()) {
      ...
      if (Ops[OpNo]->getType()->isByteOrByteVectorTy())
        Ops[OpNo] = Builder.CreateExactByteCastToInt(Ops[OpNo]);
      Ops[OpNo] = EmitSVEDupX(Ops[OpNo]);
    }
...
Value *CodeGenFunction::EmitAArch64BuiltinExpr(unsigned BuiltinID,
  ...
  if (BuiltinID == AArch64::BI__addx18byte ||
    ...
    // Add values
    if (Load->getType()->isByteOrByteVectorTy())
      Load = Builder.CreateExactByteCastToInt(Load);
    if (ValToAdd->getType()->isByteOrByteVectorTy())
      ValToAdd = Builder.CreateExactByteCastToInt(ValToAdd);
  ...
  case NEON::BI__builtin_neon_vdupb_lane_i8:
    Ops[0] = Ops[0]->getType()->isByteOrByteVectorTy()
        ? Builder.CreateExactByteCast(Ops[0],
  ...
  case NEON::BI__builtin_neon_vcvt_bf16_f32: {
    ...
    llvm::Value *Op = Ops[0]->getType()->isByteOrByteVectorTy()
      ? Builder.CreateExactByteCast(Ops[0], V4F32)
  ...
  case NEON::BI__builtin_neon_vcvtq_high_bf16_f32: {
    ...
    llvm::Value *Op0 = Ops[0]->getType()->isByteOrByteVectorTy()
      ? Builder.CreateExactByteCast(Ops[0], V8BF16)
    ...
    llvm::Value *Op1 = Ops[1]->getType()->isByteOrByteVectorTy()
      ? Builder.CreateExactByteCast(Ops[1], V4F32)
  ...
  case NEON::BI__builtin_neon_vpadalq_v: {
    ...
    llvm::Value *addend = Builder.CreateExactByteCast(Ops[0], tmp->getType());
  ...
  case NEON::BI__builtin_neon_vrsraq_n_v: {
    ...
    if (Ops[0]->getType()->isByteOrByteVectorTy())
      Ops[0] = Builder.CreateExactByteCastToInt(Ops[1]);
    Ops[0] = Builder.CreateBitCast(Ops[0], VTy);
  ...
  case NEON::BI__builtin_neon_vld3q_lane_v: {
    ...
    if (Ops[3]->getType()->isByteOrByteVectorTy())
      Ops[3] = Builder.CreateExactByteCastToInt(Ops[3]);
  ...
  case NEON::BI__builtin_neon_vld4q_lane_v: {
    ...
    if (Ops[4]->getType()->isByteOrByteVectorTy())
      Ops[4] = Builder.CreateExactByteCastToInt(Ops[4]);
