...
static Value *getMaskVecValue(CodeGenFunction &CGF, Value *Mask,
  ...
  Value *MaskVec = Mask->getType()->isByteOrByteVectorTy()
    ? CGF.Builder.CreateExactByteCast(Mask, MaskTy)
...
static Value *EmitX86FunnelShift(CodeGenFunction &CGF, Value *Op0, Value *Op1,
  ...
  if (Op0->getType()->isByteOrByteVectorTy())
    Op0 = CGF.Builder.CreateExactByteCastToInt(Op0);
  if (Op1->getType()->isByteOrByteVectorTy())
    Op1 = CGF.Builder.CreateExactByteCastToInt(Op1);
...
static Value *EmitX86MaskedCompare(CodeGenFunction &CGF, unsigned CC,
  ...
  if (CC == 3) {
    ...
    Cmp = CGF.Builder.CreateICmp(Pred, Ops[0]->getType()->isByteOrByteVectorTy() ? CGF.Builder.CreateExactByteCastToInt(Ops[0]) : Ops[0], Ops[1]->getType()->isByteOrByteVectorTy() ? CGF.Builder.CreateExactByteCastToInt(Ops[1]) : Ops[1]);
  }
