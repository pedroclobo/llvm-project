...
unsigned char test_wc_b8(unsigned char b1, unsigned char b2) {
  unsigned char o;
  asm("crand %0, %1, %2" : "=wc"(o) : "wc"(b1), "wc"(b2) : );
  return o;
// CHECK-LABEL: zeroext b8 @test_wc_b8(b8 noundef zeroext %b1, b8 noundef zeroext %b2)
// CHECK: call b8 asm "crand $0, $1, $2", "=^wc,^wc,^wc"(b8 %b1, b8 %b2)
}
...
// CHECK: call void asm sideeffect "dcbz ${0:y}\0A", "*Z,~{memory}"(ptr elementtype(b8) %addr)
}
...
// CHECK: call void asm sideeffect "dcbz ${0:y}\0A", "*Z,~{memory}"(ptr elementtype(b8) %[[VAL]])
}
