...
// COMMONIR:      [[TMP0:%.*]] = bitcast <2 x float> %a to <8 x b8>
// COMMONIR:      [[TMP1:%.*]] = bytecast exact <8 x b8> [[TMP0]] to <8 x i8>
// COMMONIR:      [[VRNDI1:%.*]] = bitcast <8 x i8> [[TMP1]] to <2 x float>
// UNCONSTRAINED: [[VRNDI1_I:%.*]] = call <2 x float> @llvm.nearbyint.v2f32(<2 x float> [[VRNDI1]])
// CONSTRAINED:   [[VRNDI1_I:%.*]] = call <2 x float> @llvm.experimental.constrained.nearbyint.v2f32(<2 x float> [[VRNDI1]], metadata !"round.tonearest", metadata !"fpexcept.strict")
...
// COMMONIR:      [[VRNDI1:%.*]] = bitcast <16 x i8> [[TMP1]] to <4 x float>
// UNCONSTRAINED: [[VRNDI1_I:%.*]] = call <4 x float> @llvm.nearbyint.v4f32(<4 x float> [[VRNDI1]])
// CONSTRAINED:   [[VRNDI1_I:%.*]] = call <4 x float> @llvm.experimental.constrained.nearbyint.v4f32(<4 x float> [[VRNDI1]], metadata !"round.tonearest", metadata !"fpexcept.strict")
// CHECK-ASM32:   vrintr.f32 s{{[0-9]+}}, s{{[0-9]+}}
// CHECK-ASM32:   vrintr.f32 s{{[0-9]+}}, s{{[0-9]+}}
// CHECK-ASM32:   vrintr.f32 s{{[0-9]+}}, s{{[0-9]+}}
// CHECK-ASM32:   vrintr.f32 s{{[0-9]+}}, s{{[0-9]+}}
