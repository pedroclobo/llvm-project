...
void test_builtin_os_log_errno(void) {
  // CHECK-NOT: @stacksave
  // CHECK: %[[BUF:.*]] = alloca [4 x b8], align 1
  // CHECK: %[[DECAY:.*]] = getelementptr inbounds [4 x b8], ptr %[[BUF]], i64 0, i64 0
...
void test_builtin_popcountg(unsigned char uc, unsigned short us,
  ...
  // CHECK-NEXT: %cast = bytecast exact b8 %2 to i8
...
void test_builtin_clzg(unsigned char uc, unsigned short us, unsigned int ui,
  ...
  // CHECK-NEXT: %cast7 = bytecast exact b8 %16 to i8
  // CHECK-NEXT: %cast8 = zext i8 %cast7 to i32
  // CHECK-NEXT: %cast9 = bytecast exact b8 %15 to i8
  // CHECK-NEXT: %iszero = icmp eq i8 %cast9, 0
  // CHECK-NEXT: %17 = load b8, ptr %sc.addr, align 1
  // CHECK-NEXT: %conv = bytecast exact b8 %17 to i8
  // CHECK-NEXT: %conv10 = sext i8 %conv to i32
  // CHECK-NEXT: %clzg = select i1 %iszero, i32 %conv10, i32 %cast8
  ...
  // CHECK-NEXT: %cast11 = zext i16 %19 to i32
  // CHECK-NEXT: %iszero12 = icmp eq i16 %18, 0
  // CHECK-NEXT: %20 = load b8, ptr %uc.addr, align 1
  // CHECK-NEXT: %conv13 = bytecast exact b8 %20 to i8
  // CHECK-NEXT: %conv14 = zext i8 %conv13 to i32
  // CHECK-NEXT: %clzg15 = select i1 %iszero12, i32 %conv14, i32 %cast11
  // CHECK-NEXT: store volatile i32 %clzg15, ptr %lz, align 4
  lz = __builtin_clzg(ui, s);
  // CHECK-NEXT: %21 = load i32, ptr %ui.addr, align 4
  // CHECK-NEXT: %22 = call i32 @llvm.ctlz.i32(i32 %21, i1 true)
  // CHECK-NEXT: %iszero16 = icmp eq i32 %21, 0
  // CHECK-NEXT: %23 = load i16, ptr %s.addr, align 2
  // CHECK-NEXT: %conv17 = sext i16 %23 to i32
  // CHECK-NEXT: %clzg18 = select i1 %iszero16, i32 %conv17, i32 %22
  // CHECK-NEXT: store volatile i32 %clzg18, ptr %lz, align 4
  lz = __builtin_clzg(ul, us);
  // CHECK-NEXT: %24 = load i64, ptr %ul.addr, align 8
  // CHECK-NEXT: %25 = call i64 @llvm.ctlz.i64(i64 %24, i1 true)
  // CHECK-NEXT: %cast19 = trunc i64 %25 to i32
  // CHECK-NEXT: %iszero20 = icmp eq i64 %24, 0
  // CHECK-NEXT: %26 = load i16, ptr %us.addr, align 2
  // CHECK-NEXT: %conv21 = zext i16 %26 to i32
  // CHECK-NEXT: %clzg22 = select i1 %iszero20, i32 %conv21, i32 %cast19
  // CHECK-NEXT: store volatile i32 %clzg22, ptr %lz, align 4
  lz = __builtin_clzg(ull, i);
  // CHECK-NEXT: %27 = load i64, ptr %ull.addr, align 8
  // CHECK-NEXT: %28 = call i64 @llvm.ctlz.i64(i64 %27, i1 true)
  // CHECK-NEXT: %cast23 = trunc i64 %28 to i32
  // CHECK-NEXT: %iszero24 = icmp eq i64 %27, 0
  // CHECK-NEXT: %29 = load i32, ptr %i.addr, align 4
  // CHECK-NEXT: %clzg25 = select i1 %iszero24, i32 %29, i32 %cast23
  // CHECK-NEXT: store volatile i32 %clzg25, ptr %lz, align 4
  ...
  // CHECK-NEXT: %cast26 = trunc i128 %31 to i32
  // CHECK-NEXT: %iszero27 = icmp eq i128 %30, 0
  // CHECK-NEXT: %32 = load i32, ptr %i.addr, align 4
  // CHECK-NEXT: %clzg28 = select i1 %iszero27, i32 %32, i32 %cast26
  // CHECK-NEXT: store volatile i32 %clzg28, ptr %lz, align 4
  ...
  // CHECK-NEXT: %cast29 = trunc i128 %34 to i32
  // CHECK-NEXT: %iszero30 = icmp eq i128 %33, 0
  // CHECK-NEXT: %35 = load i32, ptr %i.addr, align 4
  // CHECK-NEXT: %clzg31 = select i1 %iszero30, i32 %35, i32 %cast29
  // CHECK-NEXT: store volatile i32 %clzg31, ptr %lz, align 4
  // CHECK-NEXT: ret void
}
...
void test_builtin_ctzg(unsigned char uc, unsigned short us, unsigned int ui,
  ...
  // CHECK-NEXT: %conv = bytecast exact b8 %17 to i8
  // CHECK-NEXT: %conv10 = sext i8 %conv to i32
  // CHECK-NEXT: %ctzg = select i1 %iszero, i32 %conv10, i32 %cast8
  ...
  // CHECK-NEXT: %conv13 = bytecast exact b8 %20 to i8
  // CHECK-NEXT: %conv14 = zext i8 %conv13 to i32
  // CHECK-NEXT: %ctzg15 = select i1 %iszero12, i32 %conv14, i32 %cast11
  // CHECK-NEXT: store volatile i32 %ctzg15, ptr %tz, align 4
  ...
  // CHECK-NEXT: %23 = load i16, ptr %s.addr, align 2
  // CHECK-NEXT: %conv17 = sext i16 %23 to i32
  // CHECK-NEXT: %ctzg18 = select i1 %iszero16, i32 %conv17, i32 %22
  // CHECK-NEXT: store volatile i32 %ctzg18, ptr %tz, align 4
  ...
  // CHECK-NEXT: %26 = load i16, ptr %us.addr, align 2
  // CHECK-NEXT: %conv21 = zext i16 %26 to i32
  // CHECK-NEXT: %ctzg22 = select i1 %iszero20, i32 %conv21, i32 %cast19
  // CHECK-NEXT: store volatile i32 %ctzg22, ptr %tz, align 4
  ...
  // CHECK-NEXT: %iszero24 = icmp eq i64 %27, 0
  // CHECK-NEXT: %29 = load i32, ptr %i.addr, align 4
  // CHECK-NEXT: %ctzg25 = select i1 %iszero24, i32 %29, i32 %cast23
  // CHECK-NEXT: store volatile i32 %ctzg25, ptr %tz, align 4
  ...
  // CHECK-NEXT: %iszero27 = icmp eq i128 %30, 0
  // CHECK-NEXT: %32 = load i32, ptr %i.addr, align 4
  // CHECK-NEXT: %ctzg28 = select i1 %iszero27, i32 %32, i32 %cast26
  // CHECK-NEXT: store volatile i32 %ctzg28, ptr %tz, align 4
  ...
  // CHECK-NEXT: %iszero30 = icmp eq i128 %33, 0
  // CHECK-NEXT: %35 = load i32, ptr %i.addr, align 4
  // CHECK-NEXT: %ctzg31 = select i1 %iszero30, i32 %35, i32 %cast29
  // CHECK-NEXT: store volatile i32 %ctzg31, ptr %tz, align 4
  // CHECK-NEXT: ret void
}
