...
void unsigned_char_add_signed_char_unsigned_char(unsigned char *LHS, unsigned char RHS) {
  ...
  // CHECK-NEXT: %[[RHSCONV:.*]] = bytecast b8 %[[RHS]] to i8
  ...
  // CHECK-NEXT: %[[LHSCONV:.*]] = bytecast b8 %[[LHS]] to i8
  // CHECK-NEXT: %[[LHSEXT:.*]] = zext i8 %[[LHSCONV]] to i32
  // CHECK-NEXT: %[[SRC:.*]] = add nsw i32 %[[LHSEXT]], %[[RHSEXT]]
  // CHECK-NEXT: %[[SRCTRUNC:.*]] = trunc i32 %[[SRC]] to i8
  // CHECK-NEXT: %[[DST:.*]] = bitcast i8 %[[SRCTRUNC]] to b8
