...
void foo(void) {
  vc8 = 1 << vc8;
// CHECK: [[t0:%.+]] = load <8 x b8>, ptr {{@.+}},
// CHECK: [[c0:%.+]] = bytecast exact <8 x b8> [[t0]] to <8 x i8>
// CHECK: shl <8 x i8> splat (i8 1), [[c0]]
  vuc8 = 1 << vuc8;
// CHECK: [[t1:%.+]] = load <8 x b8>, ptr {{@.+}},
// CHECK: [[ext:%.+]] = bytecast exact <8 x b8> [[t1]] to <8 x i8>
// CHECK: shl <8 x i8> splat (i8 1), [[ext]]
  vi8 = 1 << vi8;
...
// CHECK: shl <8 x i16> splat (i16 1), [[t5]]
  ...
  vc8 = c << vc8;
// CHECK: [[t6:%.+]] = load b8, ptr @c,
// CHECK: [[splat_splatinsert:%.+]] = insertelement <8 x b8> poison, b8 [[t6]], i64 0
// CHECK: [[splat_splat:%.+]] = shufflevector <8 x b8> [[splat_splatinsert]], <8 x b8> poison, <8 x i32> zeroinitializer
// CHECK: [[ext8:%.+]] = bytecast exact <8 x b8> [[splat_splat]] to <8 x i8>
// CHECK: [[t7:%.+]] = load <8 x b8>, ptr {{@.+}},
// CHECK: [[ext9:%.+]] = bytecast exact <8 x b8> [[t7]] to <8 x i8>
// CHECK: shl <8 x i8> [[ext8]], [[ext9]]
  vuc8 = i << vuc8;
...
// CHECK: [[conv0:%.+]] = bytecast exact <8 x b8> [[splat_splat8]] to <8 x i8>
// CHECK: [[t9:%.+]] = load <8 x b8>, ptr {{@.+}},
// CHECK: [[conv1:%.+]] = bytecast exact <8 x b8> [[t9]] to <8 x i8>
// CHECK: shl <8 x i8> [[conv0]], [[conv1]]
  vi8 = uc << vi8;
// CHECK: [[t10:%.+]] = load b8, ptr @uc,
// CHECK: [[conv2:%.+]] = bytecast exact b8 [[t10]] to i8
...
// CHECK: [[shl22:%.+]] = shl <8 x i16> splat (i16 1), [[t16]]
 ...
 vc8 = vc8 << vc8;
// CHECK: [[t17:%.+]] = load <8 x b8>, ptr {{@.+}},
// CHECK: [[ext32:%.+]] = bytecast exact <8 x b8> [[t17]] to <8 x i8>
// CHECK: [[t18:%.+]] = load <8 x b8>, ptr {{@.+}},
// CHECK: [[ext33:%.+]] = bytecast exact <8 x b8> [[t18]] to <8 x i8>
// CHECK: shl <8 x i8> [[ext32]], [[ext33]]
  vi8 = vi8 << vuc8;
// CHECK: [[t19:%.+]] = load <8 x i32>, ptr {{@.+}},
// CHECK: [[t20:%.+]] = load <8 x b8>, ptr {{@.+}},
// CHECK: [[conv3:%.+]] = bytecast exact <8 x b8> [[t20]] to <8 x i8>
// CHECK: [[shprom:%.+]] = zext <8 x i8> [[conv3]] to <8 x i32>
// CHECK: shl <8 x i32> [[t19]], [[shprom]]
  vuc8 = vuc8 << vi8;
// CHECK: [[t21:%.+]] = load <8 x b8>, ptr {{@.+}},
// CHECK: [[conv4:%.+]] = bytecast exact <8 x b8> [[t21]] to <8 x i8>
