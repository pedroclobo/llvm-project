...
public:
  ConstantByte(const ConstantInt &) = delete;
  ...
  /// type. If the type is wider than 64 bits, the value will be zero-extended
  /// to fit the type.
  /// Get a ConstantByte for a specific value.
  static ConstantByte *get(ByteType *Ty, uint64_t V);

  /// Return a ConstantByte with the specified value and an implied Type. The
  /// type is the byte type that corresponds to the bit width of the value.
  static ConstantByte *get(LLVMContext &Context, const APInt &V);

  /// Return a ConstantByte constructed from the string strStart with the given
  /// radix.
  static ConstantByte *get(ByteType *Ty, StringRef Str, uint8_t Radix);
...
/// elements are just simple data values (i.e. ConstantInt/ConstantFP).  This
/// Constant node has no operands because it stores all of the elements of the
/// constant as densely packed data, instead of as Value*'s.
///
...
public:
  ...
  /// If this is a sequential container of integers (of any size), return the
  /// specified element in the low bits of a uint64_t.
  uint64_t getElementAsByte(unsigned i) const;
...
public:
  ...
  static Constant *getByte(Type *ElementType, ArrayRef<uint8_t> Elts);
  static Constant *getByte(Type *ElementType, ArrayRef<uint16_t> Elts);
  static Constant *getByte(Type *ElementType, ArrayRef<uint32_t> Elts);
  static Constant *getByte(Type *ElementType, ArrayRef<uint64_t> Elts);

  /// This method constructs a CDS and initializes it with a text string.
  /// The default behavior (AddNull==true) causes a null terminator to
  /// be placed at the end of the array (increasing the length of the string by
  /// one more than the StringRef would normally indicate.  Pass AddNull=false
  /// to disable this behavior. The default behavior (ByteString==false) returns
  /// a CDS with elements that are integers.  Pass ByteString=true to get a CDS
  /// with elements that are bytes.
  static Constant *getString(LLVMContext &Context, StringRef Initializer,
                             bool AddNull = true, bool ByteString = false);
...
public:
  ...
  static Constant *get(LLVMContext &Context, ArrayRef<float> Elt);
