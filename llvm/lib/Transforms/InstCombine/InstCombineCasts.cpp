...
InstCombinerImpl::isEliminableCastPair(const CastInst *CI1,
  ...
  // exact. Type punning could lead to a value mismtach.
  if (dyn_cast<ByteCastInst>(CI1) && dyn_cast<BitCastInst>(CI2) &&
      !CI1->isExact())
    Res = 0;

  // Don't fold bytecast(ptrtoint x) if bytecast is not exact.
  if (dyn_cast<ByteCastInst>(CI1) && dyn_cast<PtrToIntInst>(CI2) &&
      !CI1->isExact())
    Res = 0;
