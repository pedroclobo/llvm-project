...
static Value *convertValue(const DataLayout &DL, IRBuilderTy &IRB, Value *V,
  ...
  if (OldTy->isByteOrByteVectorTy()) {
    ...
    if (NewTy->isPtrOrPtrVectorTy())
      return IRB.CreateByteCast(IRB.CreateBitCast(V, DL.getBytePtrType(NewTy)),
                                NewTy, "", /*IsExact*/ true);

    if (!NewTy->isPtrOrPtrVectorTy())
      return IRB.CreateByteCast(IRB.CreateBitCast(V, DL.getByteIntType(NewTy)),
                                NewTy, "", /*IsExact*/ true);
  }
...
private:
  ...
  Value *rewriteIntegerLoad(LoadInst &LI) {
    ...
    if (Offset > 0 || NewEndOffset < NewAllocaEndOffset) {
      ...
      if (auto *BC = dyn_cast<ByteCastInst>(V)) {
        // Instead of truncating the result of the bytecast, replace the
        // original bytecast with a bytecast to the truncated type.
        auto *NewBC =
          IRB.CreateByteCast(BC->getOperand(0), ExtractTy, "", BC->isExact());
        deleteIfTriviallyDead(V);
        V =  NewBC;
      } else {
