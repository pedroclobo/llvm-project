...
private:
  ...
  void handleLoadOrStore(Type *Ty, Instruction &I, const APInt &Offset,
    ...
    // Replace a constant byte with a constant integer in a store instruction.
    // This enables store splitting.
...
findCommonType(AllocaSlices::const_iterator B, AllocaSlices::const_iterator E,
  ...
  for (AllocaSlices::const_iterator I = B; I != E; ++I) {
    ...
    // Used to lower memcpy/memmove to byte loads.
...
AllocaInst *SROA::rewritePartition(AllocaInst &AI, AllocaSlices &AS,
  ...
  if (!SliceTy && std::get<0>(CommonUseTy)) {
    TypeSize CommonUseSize = DL.getTypeAllocSize(std::get<0>(CommonUseTy));
