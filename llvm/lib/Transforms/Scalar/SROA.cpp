...
static Value *mergeTwoVectors(Value *V0, Value *V1, const DataLayout &DL,
  ...
  auto CastIfNeeded = [&](Value *&V, FixedVectorType *&VecType,
    ...
    if (EltType != NewAIEltTy) {
      ...
      V = V->getType()->isByteOrByteVectorTy() &&
                  !NewVecType->isByteOrByteVectorTy()
              ? Builder.CreateByteCast(V, NewVecType)
              : Builder.CreateBitCast(V, NewVecType);
      VecType = NewVecType;
      LLVM_DEBUG(dbgs() << "    bitcast " << DebugName << ": " << *V << "\n");
    }
  };

  CastIfNeeded(V0, VecType0, "V0");
  CastIfNeeded(V1, VecType1, "V1");
...
private:
  ...
  Value *rewriteIntegerLoad(LoadInst &LI) {
    ...
    if (V->getType()->isByteOrByteVectorTy() && VSize > LoadSize) {
      ...
      V = IRB.CreateByteCastToInt(V);
    } else {
      V = convertValue(DL, IRB, V, IntTy);
      assert(NewBeginOffset >= NewAllocaBeginOffset && "Out of bounds offset");
      uint64_t Offset = NewBeginOffset - NewAllocaBeginOffset;
      if (Offset > 0 || NewEndOffset < NewAllocaEndOffset) {
        IntegerType *ExtractTy =
            Type::getIntNTy(LI.getContext(), SliceSize * 8);
