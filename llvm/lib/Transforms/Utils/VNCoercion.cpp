...
Value *coerceAvailableValueUsingByteLoad(LoadInst *Load, LoadInst *CoercedLoad,
  ...
  // Trunc scalar load to vector type size if necessary.
  // This is needed as the bytecast only accepts equally sized types if one of
  // its operands is a vector.
  Value *CoercedVal = NewLoad;
  Value *LoadVal = NewLoad;
  if (LoadSize != CoercedLoadSize &&
      (LoadTy->isVectorTy() || CoercedTy->isVectorTy())) {
    if (CoercedTy->isVectorTy()) {
      Type *IntermTy = ByteType::getByteNTy(Ctx, CoercedLoadSize);
      Value *BC = Helper.CreateTrunc(NewLoad, IntermTy);
