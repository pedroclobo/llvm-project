...
;; Alloca for var and associated dbg.assign:
; CHECK: entry:
; CHECK-NEXT: %var.sroa.0 = alloca b64, align 8, !DIAssignID ![[id_1:[0-9]+]]
; CHECK-NEXT: #dbg_assign(i1 undef, ![[var:[0-9]+]], !DIExpression(), ![[id_1]], ptr %var.sroa.0, !DIExpression(), ![[loc:[0-9]+]])

;; Store to var and associated dbg.assigns.
; CHECK: store b64 %{{.*}}, ptr %var.sroa.0, align 8, !dbg !{{.*}}, !DIAssignID ![[id_2:[0-9]+]]
; CHECK-NEXT: #dbg_assign(b64 %{{.*}}, ![[var]], !DIExpression(DW_OP_LLVM_fragment, 0, 32), ![[id_2]], ptr %var.sroa.0, !DIExpression(), ![[loc]])
; CHECK-NEXT: #dbg_assign(b64 %{{.*}}, ![[var]], !DIExpression(DW_OP_LLVM_fragment, 32, 32), ![[id_2]], ptr %var.sroa.0, !DIExpression(DW_OP_plus, DW_OP_constu, 4), ![[loc]])
