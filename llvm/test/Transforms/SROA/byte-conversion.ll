...
; CHECK-NEXT:    [[TMP1:%.*]] = bytecast exact b32 [[X:%.*]] to i8
; CHECK-NEXT:    ret i8 [[TMP1]]
;
  %a = alloca b32
  store b32 %x, ptr %a
...
; CHECK-NEXT:    [[TMP1:%.*]] = bitcast b128 [[X:%.*]] to <2 x b64>
; CHECK-NEXT:    [[TMP2:%.*]] = bytecast exact <2 x b64> [[TMP1]] to <2 x ptr>
...
; CHECK-NEXT:    [[TMP1:%.*]] = bitcast b64 [[X:%.*]] to <4 x b16>
; CHECK-NEXT:    [[TMP2:%.*]] = bytecast exact <4 x b16> [[TMP1]] to <4 x i16>
...
; CHECK-NEXT:    [[TMP1:%.*]] = bitcast b64 [[X:%.*]] to <2 x b32>
